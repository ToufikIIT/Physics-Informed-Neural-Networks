...
for epoch in range(epochs + 1):
    ...
    ni = 400
    x_i = np.random.uniform(-1, 1, (1, ni))
    y_i = np.random.uniform(-1, 1, (1, ni))
...
# --- Configuration ---
target_nu = 0.0001   # The goal: microscopic shock
start_nu = 0.1       # Start smoother (0.2 was a bit high, 0.1 is safer)

epochs = 30000       # Optimized for this sampling strategy
batch_size = 300     # Increased slightly to accommodate mixed sampling
lr = 1e-3
gamma = 0.95         # Slower decay to keep learning active
...
def get_exact_solution(x, y, t, nu_val):
    # Avoid overflow in tanh for very small nu
    arg = (x + y - t) / (4 * nu_val)
    # Clip argument to prevent NaN/Overflow in exp
...
net = PINN([3, 50, 50, 50, 2]) # Slightly wider layers for sharpness

print(f"Starting Training with Shock-Biased Sampling...")
print(f"Annealing Nu: {start_nu} -> {target_nu}")
...
for epoch in range(epochs + 1):
    ...
    n_shock = batch_size - n_random
    
    x_r = np.random.uniform(-1, 1, (1, n_random))
    y_r = np.random.uniform(-1, 1, (1, n_random))
    t_r = np.random.uniform(0, 0.5, (1, n_random))
    
    t_s = np.random.uniform(0, 0.5, (1, n_shock))
    x_s = np.random.uniform(-1, 1, (1, n_shock))
    
    shock_width = 4.0 * current_nu 
    noise = np.random.uniform(-shock_width, shock_width, (1, n_shock))
    y_s = t_s - x_s + noise
    
    mask = (y_s >= -1) & (y_s <= 1)
    if np.sum(mask) > 0:
        x_s = x_s[mask].reshape(1, -1)
        y_s = y_s[mask].reshape(1, -1)
        t_s = t_s[mask].reshape(1, -1)
        
        x_c = np.hstack([x_r, x_s])
        y_c = np.hstack([y_r, y_s])
        t_c = np.hstack([t_r, t_s])
    else:
        x_c, y_c, t_c = x_r, y_r, t_r
    ...
    curr_batch = X_c.shape[1]
    dfu = (2/curr_batch)*f_u
    dfv = (2/curr_batch)*f_v
...
ax3 = fig.add_subplot(2, 2, 3)
cf3 = ax3.contourf(Xg, Yg, u_pred, 50, cmap='viridis') 
ax3.set_title("Predicted u (2D)")
plt.colorbar(cf3, ax=ax3)

ax4 = fig.add_subplot(2, 2, 4)
cf4 = ax4.contourf(Xg, Yg, u_true, 50, cmap='viridis')
ax4.set_title("Exact u (2D)")
plt.colorbar(cf4, ax=ax4)
...
plt.suptitle("1D Cross-Sections: Captured Step via Biased Sampling", fontsize=16)
plt.tight_layout()
plt.show() """
...
print(f"Starting 2D PINN training (nu={nu})...")
...
plt.suptitle(f"2D Burgers Equation Solution (u-component only, t={t_val})", fontsize=16)
N_slice = 100
x_slice = np.linspace(-1, 1, N_slice)
y_slice = np.zeros_like(x_slice)
...
plt.suptitle("1D Cross-Sections of u: Initial vs Evolved State", fontsize=16)
plt.tight_layout()
plt.show()
