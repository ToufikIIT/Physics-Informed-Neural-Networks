...
class Tanh:
    ...
    def backward(self, d_a, d_ax, d_at, d_axx):
        ...
        d_z = (d_a * d_tanh(self.z)) + \
              (d_ax * dd_tanh(self.z) * self.z_x) + \
              (d_at * dd_tanh(self.z) * self.z_t) + \
              (d_axx * (ddds * self.z_x**2 + dd_tanh(self.z) * self.z_xx))

        d_zx = (d_ax * d_tanh(self.z)) + \
               (d_axx * 2 * dd_tanh(self.z) * self.z_x)

        d_zt = d_at * d_tanh(self.z)
        d_zxx = d_axx * d_tanh(self.z)
...
print("Training PINN for Burgers with |x| initial condition...")
...
for epoch in range(epochs + 1):
    ...
    # Initial condition
    x_i = np.random.uniform(-1, 1, (1, 500))
    ...
    grads_i = net.backward((2/500)*diff_i,
                           np.zeros_like(diff_i),
                           np.zeros_like(diff_i),
                           np.zeros_like(diff_i))

    # Boundary condition
    t_b = np.random.uniform(0, 1, (1, 50))
    ...
    u_true_b = np.ones_like(u_pred_b)

    diff_b = u_pred_b - u_true_b
    loss_b = np.mean(diff_b**2)

    grads_b = net.backward((2/100)*diff_b,
                           np.zeros_like(diff_b),
                           np.zeros_like(diff_b),
                           np.zeros_like(diff_b))
    ...
    if epoch % 2000 == 0:
        print(f"Epoch {epoch} | Total Loss: {loss_f + loss_i + loss_b:.6f}")
...
for i, t_val in enumerate(test_times):
    ...
    X_in = np.vstack([x_test.reshape(1,-1),
                      t_in.reshape(1,-1)])
    ...
    plt.subplot(1,3,i+1)
    if t_val == 0:
        plt.plot(x_test, np.abs(x_test), 'k-', label='|x| (IC)')
    plt.plot(x_test, u_pred.flatten(), 'r--', label='PINN')
    plt.title(f"t = {t_val}")
    plt.grid(True)
    if i == 0:
        plt.legend()

plt.suptitle("Viscous Burgers with |x| Initial Condition")
plt.tight_layout()
plt.show()
