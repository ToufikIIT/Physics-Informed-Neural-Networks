...
print("Training PINN for Burgers with sign(x) initial condition...")
...
for epoch in range(epochs + 1):
    ...
    u_true_b = np.hstack([-np.ones((1,50)), np.ones((1,50))])
...
for i, t_val in enumerate(test_times):
    ...
    #plt.subplot(1,3,i+1)
    if t_val == 0:
        plt.plot(x_test, np.sign(x_test), 'k-', label='sign(x)')
...
plt.suptitle("Viscous Burgers with sign(x) Initial Condition")
plt.tight_layout()
plt.show()
